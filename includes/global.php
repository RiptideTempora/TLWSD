...
if(!defined('GLOBAL_LOADED')) {
  if(!session_id()) { 
    ...
    function shredData($file)
    {
		...
		// BEGIN WIPING
        if (!file_put_contents($file, str_repeat(chr(246), $l))) {
            return false;
		}
        if (!file_put_contents($file, str_repeat(chr(0), $l))) {
            return false;
		}
		if (!file_put_contents($file, str_repeat(chr(255), $l))) {
            return false;
		}
		if (!file_put_contents($file, openssl_random_pseudo_bytes($l))) {
            return false;
		}	
        if (!file_put_contents($file, str_repeat(chr(0), $l))) {
            return false;
    ...
    function removeXSS($in)
    {
		...
		if (empty($matches)) {
				return null; // Not matched to a URL
		}
        $protocol = $matches[1];
        ...
        return  $protocol . '://' . htmlspecialchars($matches[2], ENT_QUOTES | ENT_HTML5, 'UTF-8');
